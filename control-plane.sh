...
# This script can read the version from an environment variable to override the default version
...
then
    ...
    then
        ...
        # Install the right docker version has to verify if xenial or bionic
                if  [ ${ubuntu_version} -le 4 ]
        then
...
then
    ...
    sudo yum install -y docker-ce-${docker_version}-3.el7 docker-ce-cli.x86_64-${docker_version}-3.el7 containerd.io
...
then
    ...
    then
        ...
        scp ${varscphost}:vmware-kubernetes-${K8S_VERSION}.tar.gz vmware-kubernetes-${K8S_VERSION}.tar.gz
    else
...
echo "Is this an HA setup? (y/n)"

read varhacontrolplane

if [ $varhacontrolplane == n ]
then
    echo "Using Calico? (y/n)"
    read varcalico
...
EOF
        ...
        echo "Please enter your pod cidr i.e 192.168.0.0/16"
...
elif [ $varhacontrolplane == y ]
then
        echo "Please enter the Load Balancer DNS name?"

        read lbdns

        echo "What port is this on? Default is 6443"
...
networking:
    podSubnet: "192.168.0.0/16"
controlPlaneEndpoint: "${lbdns}:${lbport}"
EOF

        sudo kubeadm init --config kubeadm-config.yaml --ignore-preflight-errors=ImagePull --upload-certs
    else
        echo "Please enter your pod cidr i.e 192.168.0.0/16"
...
echo "Use the worker node script for the other masters"
    fi
fi

echo "Only run this on your first control-plane node. Each subsequent node should use the worker-node.sh script"
